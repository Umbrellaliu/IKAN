...
from isegm.data.datasets.BUSI import BUSIDataset
...
def init_model(cfg):
    ...
    model = UKANModel_hicam_fusion_mfp(num_classes=1, input_channels=3, deep_supervision=False, img_size=224, patch_size=16, in_chans=3, embed_dims=[256, 320, 512], no_kan=False,
    drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm, depths=[1, 1, 1],use_rgb_conv=True, use_disks=True, norm_radius=5, with_prev_mask=True,N=model_cfg.mfp_N,R_max=model_cfg.mfp_R_max)
...
def train(model, cfg, model_cfg):
    ...
    trainset = BUSIDataset(  # Use OCTDataset instead of SBDDataset
        cfg.BUSI_PATH,  # Make sure the path is correct
        split='train',
    ...
    valset = BUSIDataset(  # Use OCTDataset for validation set
        cfg.BUSI_PATH,  # Make sure the path is correct
        split='val',
